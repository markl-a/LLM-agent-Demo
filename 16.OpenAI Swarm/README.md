...
### 基礎入門 (1-5)
1. **[多 Agent 協作](01_多Agent協作.py)** - Swarm 基礎、Agent 創建、協作模式
2. **[Agent 切換](02_Agent切換.py)** - Handoff 機制、切換函數、路由控制
3. **[上下文變量](03_上下文變量.py)** - Context Variables、狀態共享、數據傳遞
4. **[函數工具](04_函數工具.py)** - 函數定義、工具調用、能力擴展
5. **[對話歷史](05_對話歷史.py)** - 消息管理、歷史維護、上下文保持

### 核心功能 (6-10)
6. **[路由邏輯](06_路由邏輯.py)** - 智能路由、條件判斷、Agent 分發
7. **[狀態管理](07_狀態管理.py)** - 會話狀態、持久化、狀態恢復
8. **[錯誤處理](08_錯誤處理.py)** - 異常捕獲、錯誤恢復、降級策略
9. **[流式響應](09_流式響應.py)** - Stream 模式、實時輸出、逐 token 顯示
10. **[並發 Agent](10_並發Agent.py)** - 多線程、並行處理、性能優化

### 進階應用 (11-15)
11. **[自定義 Agent](11_自定義Agent.py)** - Agent 擴展、自定義行為、高級配置
12. **[工作流程](12_工作流程.py)** - 多步驟流程、順序執行、流程編排
13. **[性能優化](13_性能優化.py)** - 緩存策略、Token 優化、響應加速
14. **[日誌記錄](14_日誌記錄.py)** - 執行追蹤、調試日誌、監控數據
15. **[測試示例](15_測試示例.py)** - 單元測試、集成測試、測試最佳實踐

### 實戰場景 (16-20)
16. **[多輪對話](16_多輪對話.py)** - 對話管理、上下文保持、會話控制
17. **[意圖識別](17_意圖識別.py)** - 意圖分類、智能路由、自動分發
18. **[監控儀表板](18_監控儀表板.py)** - 性能監控、實時統計、可視化展示
19. **[部署配置](19_部署配置.py)** - 生產部署、環境配置、容器化
20. **[最佳實踐](20_最佳實踐.py)** - 設計原則、優化技巧、常見陷阱

> 💡 **提示**: 所有示例都包含完整的代碼、詳細註釋和實用技巧，展示了 Swarm 輕量級多 Agent 編排的強大能力。
